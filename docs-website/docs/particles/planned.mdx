...
### Some Examples (as proposed by fundale):
**For the Visualizer:**

- Basic Color Brighten or Darken using `ALPASS_AUDIOLINK` or `ALPASS_FILTEREDAUDIOLINK`
- Lerp between two Colors using `ALPASS_AUDIOLINK` or `ALPASS_FILTEREDAUDIOLINK`
- Brighten / Darken from *World Origin* or *Avatar Origin* using `ALPASS_AUDIOLINKHISTORY`
- Lerp between two Colors from *World Origin* or *Avatar Origin* using `ALPASS_AUDIOLINKHISTORY`
- Change Color Hue based off `ALPASS_CHRONOTENSITY`
- Use a Theme Color using `ALPASS_THEME_COLOR0` / `ALPASS_THEME_COLOR1` / `ALPASS_THEME_COLOR2` / `ALPASS_THEME_COLOR3`
- ColorChord strip from *World Origin* or *Avatar Origin* using `ALPASS_CCSTRIP`

**For the Simulator:**

- Attract / Repulse using `ALPASS_AUDIOLINK` or `ALPASS_FILTEREDAUDIOLINK`
- Speed up / slow down the Attraction or / Repulsion using `ALPASS_AUDIOLINK` or `ALPASS_FILTEREDAUDIOLINK`
- Shockwave Repulse using `ALPASS_AUDIOLINKHISTORY`

### Other Notes:

- in addition to changing the color and strength, you could also change the size, rotation or more
- values like `ALPASS_AUDIOLINKHISTORY` or `ALPASS_CCSTRIP` could be used in combination with other planned features like **Shockwave / Pulse Effects** or **Particle Chains / Trails**
- either have a separate audio link section in the settings or add a special audio link section to every place where it would make sense
